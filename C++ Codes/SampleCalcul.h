...
public:
    ...
    void calculate()
    {
        mean.resize((int)sampleData[0].size()); //mean «2n
        mean = sampleData[0];
        for (int i = 1; i < (int)sampleData.size(); i++) //(int)sampleData.size()
        {
        ...
        stdev = stdev / (double)(sampleData.size() - 1); //sampleData.size()£∫40
        stdev = stdev.sqrtV(); 
    }
